...
const GOOGLE_SCRIPT_URL =
  'https://script.google.com/macros/s/AKfycbxC9gdvc7l5-KMPCFK9HjlS2Jq6IlWKWQ3jSeYmd3SPcmKArkM-m0hchfiB_84uTkJ54A/exec';
...
async function fetchSheet(paramName: string): Promise<any[]> {
  ...
  if (!Array.isArray(data))
    throw new Error('La API no devolvió una lista de filas');
  return data;
}
...
function mapRow(
  ...
  Object.entries(raw).forEach(([h, val]) => {
    ...
    for (const field in alias) {
      if (alias[field].includes(nk)) {
        obj[field] = cleanVal(val);
        break;
      }
    }
  });
...
const mapVenta = (r: RawRow, idx: number): Venta => {
  ...
  const totalCandidate =
    o.totalNf !== undefined && String(o.totalNf).trim() !== ''
      ? o.totalNf
      : o.valorVenta;
  return {
    id: o.id,
    ...
    lote: o.lote || '',
    origen: o.origen || '',
    unidades: toNumber(o.unidades),
    kilos: toNumber(o.kilos),
    valorKilo: toNumber(o.valorKilo),
    total: toNumber(totalCandidate),
    expedicao: o.expedicao || '',
    fechaEntrega: toDateOnly(o.fechaEntrega),
    metodoPago: o.metodoPago || '',
    pago: o.pago || '',
    fechaVencimiento: toDateOnly(o.fechaVencimiento),
    fechaPago: toDateOnly(o.fechaPago),
    facturado: o.facturado || '',
    detalle: o.detalle || '',
    estado: deriveEstadoVenta(o.pago || ''),
  };
};
...
const mapFlujoRow = (r: RawRow, idx: number): FlujoRow => {
  ...
  return {
    mes: o.mes || '',
    ingresos: toNumber(o.ingresos),
    egresos: toNumber(o.egresos),
  };
};
...
function mapMmppRow(raw: RawRow, id: string): MmppNota {
  ...
  Object.entries(raw).forEach(([h, val]) => {
    ...
    if (m) {
      ...
      if (m[2] === 'vencimiento' || m[2] === 'venc')
        boletoData[n].fechaVencimiento = cleanVal(val);
      else if (m[2] === 'valor') boletoData[n].valor = toNumber(val);
      else if (m[2] === 'estado')
        boletoData[n].estado = normalizeEstadoBoleto(val);
    }
  });
  ...
  return {
    ...
    fechaEmision: toDateOnly(obj.fechaEmision),
    cantidadKg: toNumber(obj.cantidadKg),
    valorNota: toNumber(obj.valorNota),
    boletos,
  };
}
...
const VENTAS_SEED: Venta[] = [
  {
    id: 'v1',
    fecha: addDays(-6),
    vendedor: 'Ricardo Battistini',
    cliente: 'Sin especificar',
    tipo: 'NFC-e',
    documento: '',
    producto: 'Salmão defumado fatiado COHO 100 gr',
    lote: '',
    origen: '',
    unidades: 20,
    kilos: 2,
    valorKilo: 238,
    total: 476,
    expedicao: 'Retiro en vendedor',
    fechaEntrega: addDays(-6),
    metodoPago: 'Dinheiro',
    pago: 'SIM',
    fechaVencimiento: '',
    fechaPago: '',
  ...
  },
  {
    ...
    tipo: 'no_especifica',
    documento: '',
    producto: 'Ostra cozida defumada 100gr',
    lote: '',
    origen: '',
    unidades: 10,
    kilos: 1,
    ...
    total: 270,
    expedicao: 'Retiro en vendedor',
    fechaEntrega: addDays(-5),
    metodoPago: 'no_especifica',
    pago: 'no_especifica',
    fechaVencimiento: '',
    fechaPago: '',
    facturado: 'nao',
  ...
  },
  {
    ...
    tipo: 'no_especifica',
    documento: '',
    producto: 'Mexilhão cozido defumado 100gr',
    lote: '',
    origen: '',
    unidades: 10,
    kilos: 1,
...
const INVENTARIO_SEED: InventarioItem[] = [
  {
    id: 'i1',
    producto: 'Salmón Ahumado en Frío 500g',
    categoria: 'Salmón',
    cantidad: 120,
    unidad: 'un',
  },
  {
    id: 'i2',
    producto: 'Salmón Ahumado en Frío 1kg',
    categoria: 'Salmón',
    cantidad: 45,
    unidad: 'un',
  },
  {
    id: 'i3',
    producto: 'Trucha Ahumada Filete',
    categoria: 'Trucha',
    cantidad: 0,
    unidad: 'un',
  },
  {
    id: 'i4',
    producto: 'Mejillones Ahumados en Aceite',
    categoria: 'Mariscos',
    cantidad: 60,
    unidad: 'frasco',
  },
...
const COMPRAS_MMPP_SEED: MmppNota[] = [
  {
    id: 'm1',
    notaFiscal: 'FC-5521',
    proveedor: 'Pesquera Los Fiordos',
    producto: 'Salmón entero fresco',
    fechaEmision: addDays(-15),
    cantidadKg: 2200,
    valorNota: 9680000,
    boletos: [
...
const PAGOS_SEED: Pago[] = [
  {
    id: 'p1',
    mes: monthKey(-1),
    responsable: 'Roberto Alvarez',
    detalle: 'Compra de insumos de embalaje',
    monto: 1250000,
  },
];
...
const STORAGE_KEYS: Record<string, string> = {
  ...
  mmpp: 'panel:compras-mmpp',
  pagos: 'panel:pagos',
  flujo: 'panel:flujo-caja',
};
...
async function loadModule<T>(
  storageKey: string,
  sheetParamKey: string,
  mapFn: (r: RawRow, idx: number) => T,
  seed: T[]
...
async function loadMmpp(seed: MmppNota[]): Promise<LoadResult<MmppNota>> {
  try {
    ...
    const mapped = raw.map((r: RawRow, idx: number) =>
      mapMmppRow(r, 'm-' + idx)
    );
...
function BrandMark({ size = 28 }: { size?: number }) {
  return (
    <div
      ...
      className="flex items-center justify-center text-white font-semibold text-sm flex-shrink-0"
    >
      P
    </div>
  );
...
function Sidebar({
  ...
  return (
    <aside
      style={{ background: C.navy, width: 232, flexShrink: 0 }}
      className="flex flex-col py-5"
    >
      <div
        className="px-4 pb-5 mb-2"
      ...
      <div
        ...
        className="px-4 text-xs mb-1 mt-2"
      >
        General
      </div>
      ...
      {role === 'gerencia' && (
        <>
          <div
            style={{ color: 'rgba(255,255,255,0.4)' }}
            className="px-4 text-xs mb-1 mt-5"
          >
...
function AuthScreen({
  ...
  return (
    ...
    >
      <div
        style={{ border: `1px solid ${C.border}`, background: C.surface }}
        className="rounded-md p-8 w-full"
        style={{ maxWidth: 380 }}
      >
        ...
        {!target ? (
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={() => pick('gerencia')}
                style={{ background: C.navy }}
                className="px-4 py-2.5 rounded-md text-sm text-white"
              >
          ...
          <>
            <div
              className="text-sm mb-1 text-center font-medium"
              style={{ color: C.text }}
            >
              {target === 'gerencia'
                ? 'Gerencia / Administrador'
...
function TopBar({
  ...
  const elapsed = lastSync
    ? Math.max(0, Math.round((Date.now() - lastSync.getTime()) / 1000))
    : null;
  const [label, color] =
    STATUS_LABEL[connectionStatus] || STATUS_LABEL.checking;
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {notifOpen && (
            <div
              style={{ border: `1px solid ${C.border}`, background: C.surface }}
              className="absolute right-0 mt-2 w-80 rounded-md shadow-lg z-20 max-h-96 overflow-auto"
            >
              <div
...
function VentasView({ ventas }: { ventas: Venta[] }) {
  ...
  const meses = [
    'Todos',
    ...new Set(ventas.map((v) => (v.fecha || '').slice(0, 7))),
  ]
    .filter(Boolean)
    .sort();
  ...
  const rowStyle = (estado: Venta['estado']): React.CSSProperties => {
    if (estado === 'Pagada')
      return { background: C.successBg, color: '#1F5C3B' };
    if (estado === 'No pagada')
      return { background: C.dangerBg, color: '#8B2E24' };
  ...
  return (
    <div>
      <ModuleHeader
        title="Ventas"
        subtitle="Una fila por producto vendido — filtra por vendedor, cliente o mes (histórico desde noviembre de 2025)"
      >
        <select
          value={vendedorF}
      ...
      <ColorLegend
        items={[
          ['Pagada', C.successBg, '#1F5C3B'],
          ['No pagada', C.dangerBg, '#8B2E24'],
          ['Sin definir', C.neutralBg, C.textMuted],
        ]}
      />
    </div>
  );
...
function ComisionesView({ comisiones }: { comisiones: Comision[] }) {
  ...
  const filtered = comisiones.filter(
    (c) =>
      (vendedorF === 'Todos' || c.vendedor === vendedorF) &&
      (mesF === 'Todos' || c.mes === mesF)
  );
  ...
  return (
    <div>
      <ModuleHeader
        ...
        subtitle="Filtra por vendedor y/o por mes"
      >
        <select
          value={vendedorF}
          onChange={(e) => setVendedorF(e.target.value)}
...
function InventarioView({ inventario }: { inventario: InventarioItem[] }) {
  const rowStyle = (item: InventarioItem): React.CSSProperties =>
    item.cantidad > 0 ? { background: C.successBg } : { background: '#FFFFFF' };
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {inventario.map((p) => (
              <tr
                key={p.id}
                style={{ ...rowStyle(p), borderTop: `1px solid ${C.border}` }}
              >
      ...
      <ColorLegend
        items={[
          ['Con disponibilidad', C.successBg, '#1F5C3B'],
          ['Sin disponibilidad', '#FFFFFF', C.border],
        ]}
      />
    </div>
  );
...
function MmppView({ mmpp }: { mmpp: MmppNota[] }) {
  ...
  return (
    <div>
      <ModuleHeader
        title="Compras de materia prima (MMPP)"
        subtitle="Notas más recientes arriba, las más antiguas abajo"
      />
          ...
          >
            <div className="flex flex-wrap items-baseline justify-between gap-2 mb-3">
              <div>
                ...
                <span style={{ color: C.textMuted }} className="ml-2 text-xs">
                  {n.proveedor} · {n.producto} · {n.cantidadKg} kg · emitida{' '}
                  {n.fechaEmision}
                </span>
              </div>
            ...
            {n.boletos.length > 0 && (
              <div
                ...
                style={{
                  gridTemplateColumns: `repeat(${n.boletos.length}, 1fr)`,
                }}
              >
                {n.boletos.map((b, idx) => (
                  <div
                    ...
                    className="rounded px-3 py-2 text-xs flex items-center justify-between"
                  >
                    <div>
                      <div style={{ color: C.textMuted }}>
                        Boleto {idx + 1} · vence {b.fechaVencimiento}
                      </div>
...
function PagosView({ pagos }: { pagos: Pago[] }) {
  ...
  filtered.forEach((p) => {
    porResponsable[p.responsable] =
      (porResponsable[p.responsable] || 0) + (Number(p.monto) || 0);
  });
  ...
  return (
    <div>
      <ModuleHeader
        ...
        subtitle="Gastos mensuales y responsable"
      >
        <select
          value={mesF}
          onChange={(e) => setMesF(e.target.value)}
        ...
        {Object.entries(porResponsable).map(([r, v]) => (
          ...
          >
            <div className="flex items-center gap-1.5">
              <span
                style={{ background: RESPONSABLE_COLOR[r] || C.textMuted }}
                className="w-2 h-2 rounded-full inline-block"
              />
              <span style={{ color: C.textMuted }}>{r}</span>
            </div>
            <div style={{ color: C.text }} className="font-medium mt-0.5">
              {formatCurrency(v)}
            </div>
          </div>
        ))}
  ...
  return (
    <div>
      <ModuleHeader
        ...
        subtitle="Ingresos, egresos y saldo acumulado por mes — desde la pestaña Flujo"
      />
      <div
        style={{
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.mes} style={{ borderTop: `1px solid ${C.border}` }}>
                ...
                <td className="px-3 py-2 text-right">
                  {formatCurrency(r.egresos)}
                </td>
                <td
                  className="px-3 py-2 text-right"
                  style={{ color: r.saldo >= 0 ? C.success : C.danger }}
                >
...
function InventarioMiniChart({ inventario }: { inventario: InventarioItem[] }) {
  ...
  inventario.forEach((p) => {
    porCategoria[p.categoria] =
      (porCategoria[p.categoria] || 0) + (Number(p.cantidad) || 0);
  });
...
function EstadisticasView({
  ...
  ventas.forEach((v) => {
    porVendedor[v.vendedor] =
      (porVendedor[v.vendedor] || 0) + (Number(v.total) || 0);
  });
  const dataVendedor = Object.entries(porVendedor).map(([name, value]) => ({
    name,
    value,
  }));
  ...
  const dataEstado = Object.entries(estadoCount).map(([name, value]) => ({
    name,
    value,
  }));
  const PIE_COLORS: Record<string, string> = {
    Pagada: C.success,
    'No pagada': C.danger,
    'Sin definir': C.textMuted,
  };

  const gastoPorResponsable: Record<string, number> = {};
  pagos.forEach((p) => {
    gastoPorResponsable[p.responsable] =
      (gastoPorResponsable[p.responsable] || 0) + (Number(p.monto) || 0);
  });
  const dataGasto = Object.entries(gastoPorResponsable).map(
  ...
  return (
    <div>
      <ModuleHeader
        title="Estadísticas"
        subtitle="Se calculan solas a partir de Ventas, Inventario y Pagos — no hay nada que cargar aquí"
      />
        ...
        <ChartCard title="Estado de pago de las ventas">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              >
                {dataEstado.map((d, i) => (
                  <Cell key={i} fill={PIE_COLORS[d.name] || C.textMuted} />
                ))}
              </Pie>
              <Tooltip />
            </PieChart>
...
export default function App() {
  ...
  const refreshAll = useCallback(async (isInitial: boolean) => {
    ...
    if (prevInventarioRef.current) {
      ...
      prevInventarioRef.current.forEach((item) => {
      ...
      i.forEach((item) => {
        ...
        if (!prev) {
          newEvents.push({
            id: 'evt-new-' + item.id + '-' + Date.now(),
            tipo: 'info',
            area: 'Inventario',
            texto: `Nuevo producto agregado: ${item.producto}`,
          });
        } else if (prev.cantidad === 0 && Number(item.cantidad) > 0) {
          newEvents.push({
            id: 'evt-restock-' + item.id + '-' + Date.now(),
            tipo: 'info',
            area: 'Inventario',
            texto: `Se repuso stock de ${item.producto} (${item.cantidad} ${item.unidad})`,
          });
        }
      });
      if (newEvents.length)
        setInventoryEvents((old) => [...newEvents, ...old].slice(0, 8));
    }
    prevInventarioRef.current = i;

    setVentas(vR.data);
    setComisiones(cR.data);
    setInventario(i);
  ...
  useEffect(() => {
    if (role === 'trabajador' && MENU_ADMIN.some((m) => m.id === active))
      setActive('ventas');
  }, [role, active]);
  ...
  const alerts = useMemo<Alert[]>(() => {
    ...
    ventas.forEach((v) => {
      if (v.estado !== 'Pagada') {
        ...
        if (d !== null && d <= 7) {
          const ref = v.documento
            ? `Doc. ${v.documento} de ${v.cliente}`
            : `Venta a ${v.cliente}`;
          list.push({
            id: 'venta-' + v.id,
            ...
            texto:
              d < 0
                ? `${ref} vencida hace ${Math.abs(d)} día(s)`
                : `${ref} vence en ${d} día(s)`,
          });
        }
      }
    });
    inventario.forEach((p) => {
      if (p.cantidad === 0) {
        list.push({
          ...
          tipo: 'danger',
          area: 'Inventario',
          texto: `${p.producto} está sin stock`,
        });
      }
    });
    if (role === 'gerencia') {
          ...
          if (b.estado !== 'Pagado') {
            ...
            if (d !== null && d <= 7) {
              list.push({
                ...
                texto:
                  d < 0
                    ? `Boleto ${idx + 1} de ${nota.proveedor} (Nota ${
                        nota.notaFiscal
                      }) vencido hace ${Math.abs(d)} día(s) — ${formatCurrency(
                        b.valor
                      )}`
                    : `Boleto ${idx + 1} de ${nota.proveedor} (Nota ${
                        nota.notaFiscal
                      }) vence en ${d} día(s) — ${formatCurrency(b.valor)}`,
              });
            }
          }
        });
  ...
  const renderModule = (): ReactNode => {
    switch (active) {
      case 'ventas':
        return <VentasView ventas={ventas} />;
      case 'comisiones':
      ...
      case 'mmpp':
        return role === 'gerencia' ? <MmppView mmpp={mmpp} /> : null;
      case 'pagos':
        return role === 'gerencia' ? <PagosView pagos={pagos} /> : null;
      case 'flujo':
        return role === 'gerencia' ? <FlujoCajaView flujo={flujo} /> : null;
      case 'estadisticas':
